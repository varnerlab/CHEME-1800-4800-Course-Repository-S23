...
"""
function readreactionfile(path::String)::Dict{String, MyChemicalReaction}
    
    # check: is path legit?
    # in production we would check this path, assume ok for now

    # initialize -
    reactions = Dict{String, MyChemicalReaction}()
    ...
    open(path, "r") do io # open a stream to the file
        for line in eachline(io) # read each line from the stream
            ...
            if (contains(line,"#") == false)
                ...
                # build - 
                model = build(MyChemicalReaction, name, reactant, product);
